...
class MalabAddPopUp extends StatefulWidget {

  const MalabAddPopUp({super.key});

  @override
  State<MalabAddPopUp> createState() => _MalabAddPopUpState();
}

class _MalabAddPopUpState extends State<MalabAddPopUp> {
  ...
  void initState() {
    super.initState();
    HomeCubit.get(context).featchAdMalabPopUpData();
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<HomeCubit, HomeState>(
      ...
      builder: (context, state) {
        ...
        textController = TextEditingController(text: cubit.adMalabPopUpData?.text);
        linkController = TextEditingController(text: cubit.adMalabPopUpData?.link);

        return Scaffold(
          appBar: buildAppBar(context, title: 'اعلان الملعب', showBackButton: true),
          body: Padding(
                      ...
                      child: Container(
                        ...
                        child: newImagePath != null
                          ...
                          fit: BoxFit.cover,
                            ...
                            : cubit.adMalabPopUpData != null
                            ? Image.network(
                          cubit.adMalabPopUpData?.imageUrl ?? '',
                          fit: BoxFit.cover,
                        ...
                        state is updateAdPopUpLoading ||
                          ...
                          onPressed: () {
                            ...
                            if (cubit.adMalabPopUpData == null) {
                              ...
                              // Update
                              cubit.updateMalabAdPopUp(
                                text: textController.text,
                                link: linkController.text,
                                newImagePath: newImagePath,
                                oldImageUrl: cubit.adMalabPopUpData?.imageUrl ?? '',
                              );
                            }
                          },
                          child: Text(
                            cubit.adMalabPopUpData == null ? 'اضافه' : 'تحديث',
                        ...
                        if (cubit.adMalabPopUpData != null)
                          isButtonDisabled || state is deleteAdPopUpLoading
                            ...
                            onPressed: () {
                              ...
                              cubit.deleteMalabAdPopUp(imageUrl: cubit.adMalabPopUpData?.imageUrl ?? '');
                            },
